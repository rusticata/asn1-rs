...
fn derive_berparser_tagged() {
    ...
    let (rem, res) = AATagEx1::parse_ber(input).expect("parsing failed");
    assert!(rem.is_empty());
    assert_eq!(
        res,
        AATagEx1 {
    ...
    let input = Input::from_slice(&hex!("30 06 a0 04 020200aa"));
    let (rem, res) = AATagEx2::parse_ber(input).expect("parsing failed");
    assert!(rem.is_empty());
    assert_eq!(res, AATagEx2 { a: 0xaa });
}

fn derive_berparser_opttagged() {
    //----- using type
...
fn derive_berparser_opttagged_default() {
    //----- using attribute
    ...
    let input = Input::from_slice(&hex!("30 06 a0 04 040200aa"));
    let _e = AATagEx2::parse_ber(input).expect_err("parsing failed");
}

fn main() {
    derive_berparser_tagged();
    derive_berparser_opttagged();
    derive_berparser_opttagged_default();
}
