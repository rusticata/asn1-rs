...
fn derive_berparser_default() {
    ...
    let (rem, res) = AADefault::parse_ber(input).expect("parsing failed");
    assert!(rem.is_empty());
    ...
    let input = Input::from_slice(&hex!("30 04 040200aa"));
    let (rem, res) = AADefault::parse_ber(input).expect("parsing failed");
